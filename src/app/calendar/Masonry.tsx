...
type GridItem = FootageItem & { x: number; y: number; w: number; h: number }
...
const Masonry = ({
  ...
  useEffect(() => {
    const imageItems = items.filter(i => !i.type || i.type === 'image')
    preloadImages(imageItems.map(i => i.img)).then(() => setImagesReady(true))
  }, [items])
  ...
  const handleMouseEnter = (e: React.MouseEvent, item: GridItem) => {
    ...
    if (colorShiftOnHover) {
      const overlay = (e.currentTarget as HTMLElement).querySelector('.masonry-color-overlay')
